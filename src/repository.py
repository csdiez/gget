...
def git(*args: Path | str, timeout: int = 30, **kwargs: Any) -> subprocess.CompletedProcess:
    cmd_args = ['git']
...
def git_games(*args: str, timeout: int = 30, work_tree: Path | None = None, **kwargs: Any) -> subprocess.CompletedProcess:
    if work_tree:
        return git(f'--work-tree={work_tree}', f"--git-dir={BARE_REPO}", *args, timeout=timeout, **kwargs)
    else:
        return git(f"--git-dir={BARE_REPO}", *args, timeout=timeout, **kwargs)

def git_games_output(*args, timeout: int = 30, work_tree: Path | None = None, **kwargs: Any) -> str:
    if work_tree:
        return git_output(f'--work-tree={work_tree}', f"--git-dir={BARE_REPO}", *args, timeout=timeout, **kwargs)
    else:
...
def remote_saves() -> list[str]:
    ...
    result = git_games("ls-tree", "-d", "--name-only", "origin/main",
